...
width:20%;
	...
	<script>
		<?php  
			$current_time=time();
			$stop_time=strtotime("today 00:10");
...
$path=$path_r['path'];
?>	
<body style="background-image:url('images/bg.jpg');background-repeat:no-repeat;margin:0px;background-size:contain;">
...
echo '<br><br>';
echo '<p style="text-align:center;padding-left:40px;margin-top:-1px;line-height:0.1;font-size:25px;"><b>Rankings</b></p>';

echo '<li style="overflow-y:auto;list-style-type:none">';

$count=0;
$rankings = mysqli_query($con,"select scale,username from login order by scale desc,timestamp desc");
...
{
$count++;
echo '<ol style="font-size:20px;line-height:0.5;margin-left:-35px;">'.$count.' '.$row_r['username'].'</ol>';
}
}
...
echo '<button  style="color:white;position:absolute;top:290px;left:410px;font-size:14pt;" class="pathbtn" id="path1" onclick="path(1)">Entertainmant</button>';
echo '<button  style="color:white;position:absolute;top:340px;left:410px;font-size:14pt;" id="path4" class="pathbtn" onclick="path(4)">Literature</button>';
...
echo '<button style="color:white;position:absolute;top:168px;left:360px;font-size:14pt;" id="path2" class="pathbtn" onclick="path(2)">Entertainmant</button>';
echo '<button style="color:white;position:absolute;top:205px;left:360px;font-size:14pt;" id="path8" class="pathbtn" onclick="path(8)">History</button>';
echo '<div align="centre"  id="empty_ans">';
echo '</div>';
echo '</div>';
}
...
echo '<button style="color:white;position:absolute;top:250px;left:280px;font-size:14pt;" id="path7" class="pathbtn" onclick="path(7)">History</button>';
echo '<button style="color:white;position:absolute;top:315px;left:280px;font-size:14pt;" id="path5" class="pathbtn" onclick="path(5)">Literature</button>';
echo '<div align="centre"  id="empty_ans">';
echo '</div>';
echo '</div>';
}
...
echo '<button style="color:white;position:absolute;top:350px;left:432px;font-size:12pt;" id="path3" class="pathbtn" onclick="path(3)">Entertainmant</button>';
echo '<div align="centre"  id="empty_ans">';
echo '</div>';
echo '</div>';
...
echo '<button style="color:white;position:absolute;top:310px;left:430px;font-size:14pt;" id="path6" class="pathbtn" onclick="path(6)">Literature</button>';
echo '<div align="centre"  id="empty_ans">';
echo '</div>';
echo '</div>';
...
<br><br><br>';
if($path==1 && ($current_q>=61 && $current_q<=65) )
echo '<h1 class="path">'.'You are on Entertainmant Path'.'</h1>';
if($path==2 && ($current_q>=61 && $current_q<=65))
echo '<h1 class="path">'.'You are on Entertainmant Path'.'</h1>';
if($path==3 && ($current_q>=61 && $current_q<=65))
echo '<h1 class="path">'.'You are on Entertainmant Path'.'</h1>';
if($path==4 && ($current_q>=66 && $current_q<=70))
echo '<h1 class="path">'.'You are on Literature Path'.'</h1>';
if($path==5 && ($current_q>=66 && $current_q<=70))
echo '<h1 class="path">'.'You are on Literature Path'.'</h1>';
if($path==6 && ($current_q>=66 && $current_q<=70))
echo '<h1 class="path">'.'You are on Literature Path'.'</h1>';
if($path==7 && ($current_q>=71 && $current_q<=74))
echo '<h1 class="path">'.'You are on History Path'.'</h1>';
if($path==8 && ($current_q>=71 && $current_q<=74))
echo '<h1 class="path">'.'You are on History Path'.'</h1>';
if($current_q<=3 && $current_q>=1)
echo '<h1 class="path">'.'Mind Chow'.'</h1>';
if($current_q<=6 && $current_q>=4)
echo '<h1 class="path">'.'Science & Technology'.'</h1>';
if($current_q<=10 && $current_q>=7)
echo '<h1 class="path">'.'Sports'.'</h1>';
if($current_q>=11 && $current_q<=16)
echo '<h1 class="path">'.'Geography'.'</h1>';
if($current_q>=17 && $current_q<=21)
echo '<h1 class="path">'.'SitComs'.'</h1>';
...
echo '<div style="margin-left:310px;margin-right:auto;display:block;"  id="empty_ans">';
echo '</div>';
echo '<button  style="margin-left:auto;margin-right:auto;display:block" class="btn" onclick="submit_ans('.$current_q.');">Submit</button><br>';
...
<div class="right">
	...
	<div class="container">
	<h1 style="font-size:40pt;text-align:center;">Treasure Map</h1>
		<div class="mapPanel">
		  <img  src="images/map.png"/>
		 
		  <?php 
			$y =$y - 2; 
		  echo '<span style="top:'.$y.'%;left:'.$x.'%"'.' '.'class="flag f" id="'.$current_q.'"></span>';
		  ?>
		</div>
	</div>
<?php
	echo '<script type="text/javascript">';
	if($current_q==101 || $current_q==102 || $current_q==103 || $current_q==104 || $current_q==105 || $current_q==106 || $current_q==107)		
	echo 'document.getElementById("'.$current_q.'").style.visibility="hidden"';
	else
	echo 'document.getElementById("'.$current_q.'").style.visibility="visible"';
	echo '</script>';
?>
